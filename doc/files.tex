...
\section{One resource, many handlers}
...
As you see the {\tt load} method accepts an optional parameter, a resource.
...
\section{Serialization and de-serialization}
...
Remember the method {\tt to\_str} we saw at the beginning? it is the
...
the next chapter. Now we are going to present some of the file handler
classes {\tt itools} includes.

The first one is the {\tt File} handler class, which is the root of the
inheritance tree for all file handlers. This is the one to use when there
is nothing better. It only provides the method {\tt to\_str} we have already
seen before.
...
\subsection{Text handlers}

The {\tt Text} handler class is the base class for all text handlers.
