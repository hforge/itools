...
tracker. The code can be found in the directory {\tt examples/chapter6}.
...
{\tt itools.tt}. Using our own filename extension ({\tt tt}) will prove
...
First, our handler class {\tt TaskTracker} inherits from the handler class
{\tt Text}, because it is intended to manage a text file.

The method {\tt \_load\_state} is the one to implement, it is responsible
to de-serialize the resource and build a data structure on memory that
represents it.

The first thing it does is to call the parent's {\tt \_load\_state} method,
which will read the resource's data and get a unicode string from it, which
will be stored in the {\tt self.state.data} variable. This is an intermediary
representation, which will be discarded.

The rest of the method process the raw data and builds a list of instances
of the class {\tt Task} into the variable {\tt self.state.tasks}.
...
just adding the {\tt to\_unicode} method to the {\tt TaskTracker} class:
...
The full code can be found in {\tt examples/chapter6/TaskTracker.py}.