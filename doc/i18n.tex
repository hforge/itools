...
The method {\tt gettext} does two things, it returns the message translation
when it is called, and marks the message so the  extraction script will
be able to find it.

But sometimes we only want mark the text string. How to tell the message
extraction script to pick up a message but still do nothing at run-time?
We use a dummy function:
...
The function {\tt N\_} returns the same message it receives, so it changes
nothing. But it allows the message extraction script to detect the message.
