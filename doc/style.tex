...
to these coding rules.

This guide is more strict than the recommedations of {\em PEP
8}\footnote{http://www.python.org/peps/pep-0008.html}. Sometimes this guide
contradicts the {\em PEP 8} recommendations; in these cases this guide is
applies, in the context of {\tt itools}.
...
The explanation aboyt how to write a documentation string are out of this
chapter, it is covered by the {\em PEP
...
\subsubsection{Maximum line length}
...
\begin{code}
    # Bad                           |    # Bad
    if x is True: do_something()    |    do_one(); do_two(); do_three()
                                    |
    # Good                          |    # Good
    if x is True:                   |    do_one()
        do_something()              |    do_two()
...
Surround operators with one white space.
...
A comment starts by a single {\tt \#} character followed by an space.
